        ...
        data: {
            ...
            async handler(ctx) {
                ...
                const _promotions = await ctx.call('v1.promotions.find', {
                    query: {active: true},
                    sort: 'order',
                    fields: '_id name description picture banner content'
                });
                ...
                const _seasons = await ctx.call('v1.seasons.find', {
                    query: {active: true, placement: {$ne: 'site'}},
                    sort: 'order'
                });
                ...
                const _paymentMethods = await ctx.call('v1.payment-methods.find', {
                    sort: 'order',
                    query: {active: true, placement: {$ne: 'site'}, test: isTest()}
                });
