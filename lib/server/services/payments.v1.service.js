    ...
    actions: {
        ...
        pay: {
            ...
            async handler(ctx) {
                ...
                const paymentMethod = await ctx.call('v1.payment-methods.findOne', {conditions: {_id: methodid}});
                if (!paymentMethod) {
    ...
    methods: {
        ...
        async season(payment) {
            ...
            if (season) {
                ...
                const seasons = await this.broker.call('v1.customer-seasons.customer', {customerid});
                await this.broker.call('v1.io.emit', {
                    room: `customer-${customerid}`,
                    event: 'seasons',
                    data: {seasons}
