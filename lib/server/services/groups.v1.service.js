    ...
    actions: {
        ...
        delete: {
            ...
            async handler(ctx) {
                ...
                const id = ctx.params.id;
                const group = await ctx.call('v1.groups.findOne', {conditions: {_id: id}});
                if (!group) {
                    throw new MoleculerServerError(ctx.meta.__('group-not-found'), 404);
                }
                ...
                count += await ctx.call('v1.groups.countDocuments', {filter: {parent: id}});
                count += await ctx.call('v1.products.countDocuments', {filter: {group: id}});

                if (count > 0) {
                    throw new MoleculerServerError(ctx.meta.__('group-is-not-empty'));
                }

                await ctx.call('v1.groups.deleteOne', {conditions: {_id: id}});
