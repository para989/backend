    ...
    actions: {
        ...
        profile: {
            ...
            async handler(ctx) {
                ...
                const set = {name: user.name, email: user.email, phone: user.phone, avatar: user.avatar};
                const unset = {};
                if (user.phone) {
                ...
                await ctx.call('v1.users.updateOne', {filter: {_id: user.id}, doc}).catch(() => {
                    throw new MoleculerServerError(ctx.meta.__('user-exists'));
                });
        ...
        item: {
            ...
            async handler(ctx) {
                ...
                if (!user) {
                    throw new MoleculerServerError('User not found', 404);
                }
