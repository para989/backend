    ...
    actions: {
                    ...
                    _.each(forms, form => {
                        switch (form.type) {
                            case 'user-questionnaire':
                                result.questionnaire = form.items;
                                break;
                            case 'user-profile':
        ...
        deleteUser: {
            ...
            async handler(ctx) {
                ...
                const user = await ctx.call('v1.users.findOne', {conditions: {_id: id}});
                if (!user) {
                    throw new MoleculerServerError(ctx.meta.__('user-not-found'), 404);
                }

                await ctx.call('v1.users.deleteOne', {conditions: {_id: id}});

                await this.broker.cacher.clean('v1.users.**');

            }
        },
        // v1.users.getQuestionnaire
        getQuestionnaire: {
            role: 'users:read',
            rest: 'GET /user/questionnaire/:id',
            params: {
                id: 'objectID',
            },
            async handler(ctx) {
        ...
        // v1.users.setQuestionnaire
        setQuestionnaire: {
            role: 'users:write',
            rest: 'PATCH /user/questionnaire',
            async handler(ctx) {
