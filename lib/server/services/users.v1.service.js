...
module.exports = {
    ...
    async started() {
        ...
        /*const users = await this.adapter.model.find({profile: {$exists: true}}, 'profile');
        const ops = [];
        _.each(users, user => {
            const phrases = [];
            _.each(user.profile, item => {
                phrases.push(item);
            });
            const words = morphy(_.join(phrases, ' '));
            if (words) {
                ops.push({updateOne: {filter: {_id: user.id}, update: {words}}});
            } else {
                ops.push({updateOne: {filter: {_id: user.id}, update: {$unset: {words: ''}}}});
            }
        });
        if (_.size(ops)) {
    ...
    actions: {
        ...
        getUsers: {
            ...
            async handler(ctx) {
                ...
                const filter = ctx.params.filter ? JSON.parse(ctx.params.filter) : undefined;

                const places = await ctx.call('v1.places.selector');
                const roles = await ctx.call('v1.user-roles.selector');
                ...
                const form = await ctx.call('v1.forms.findOne', {query: {type: 'user-profile'}});
                if (form) {
                ...
                const placeids = _.get(filter, 'placeids');
                if (_.size(placeids)) {
                    where.places = {$in: placeids};
                } else if (_.size(ctx.meta.user.places)) {
                    where.places = {$in: ctx.meta.user.places};
                }

                const answers = _.get(filter, 'answers');
                if (_.size(answers)) {
                    _.each(answers, (value, key) => {
                        where[`profile.${key}`] = value;
                    });
                }

                if (role) {
                    where.role = role;
                } else if (ctx.meta.user.role !== 'admin') {
                    where.role = {$ne: 'admin'};
                }
                ...
                const users = await ctx.call('v1.users.find', {
                    query: where,
                    search,
                    searchFields: 'name email phone words',
                    limit: 100,
                    sort: '-updated',
        ...
        getUser: {
            ...
            async handler(ctx) {
                ...
                result.roles = await ctx.call('v1.user-roles.selector');
                ...
                if (user.role === 'admin' && ctx.meta.user.role !== 'admin') {
    ...
    methods: {
        ...
        async update() {
            ...
            const counters = await this.adapter.model.aggregate([{$group: {_id: "$role", count: {$sum: 1}}}]).exec();
            await this.broker.emit('user:counters', {counters});
        },
    }
};
