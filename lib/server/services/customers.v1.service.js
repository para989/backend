    ...
    actions: {
        ...
        signIn: {
            ...
            async handler(ctx) {
                ...
                const name = ctx.meta.__('customer');
                const token = await generateToken();

                const customer = await ctx.call('v1.customers.findOneAndUpdate', {
                    conditions: {
                        phone,
                    },
                    update: {
                ...
                if (customer.test && customer.code) {
                    ...
                    const params = {};
                    params.service_id = _.get(this.broker.metadata, 'call.id');
                    params.key = _.get(this.broker.metadata, 'call.key');
                    params.phone = phone;
