    ...
    actions: {
        ...
        signIn: {
            ...
            async handler(ctx) {
                ...
                let exists = true;
                let customer = await this.adapter.model.findOne(conditions);
                if (!customer) {
                    exists = false;
                    customer = await this.adapter.model.findOneAndUpdate(conditions, {$setOnInsert: insert}, {upsert: true, new: true});
                }
        ...
        code: {
            ...
            async handler(ctx) {
                ...
                if (_.size(gifts) === 1) {
                    const gift = gifts[0];
                    const customerGift = await ctx.call('v1.customer-gifts.findOne', {conditions: {customer: customer._id, gift: gift._id}});
                    if (customerGift) {
