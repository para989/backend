...
module.exports = {
    ...
    events: {
        'payment:created': {
            async handler(ctx) {
                const paymentMethod = ctx.params.paymentMethod;
                await this.adapter.model.updateOne({_id: paymentMethod._id}, {$inc: {purchases: 1}});
            }
        },
    },
    async started() {
    ...
    actions: {
        ...
        delete: {
            ...
            async handler(ctx) {
                ...
                const method = await ctx.call('v1.payment-methods.findOne', {query: {_id: id}});
                if (!method) {
                    throw new MoleculerServerError(ctx.meta.__('payment-method-not-found'), 404);
                }

                await ctx.call('v1.payment-methods.deleteOne', {query: {_id: id}});

                await this.broker.emit('payment-methods:deleted', {method});
        ...
        popular: {
            role: 'reports:read',
            rest: 'GET /user/popular/payment-methods',
            cache: true,
            async handler(ctx) {
                const paymentMethods = await this.adapter.model.find({test: false}, 'id name purchases').sort('-purchases').limit(10).exec();
                const items = [];
                _.each(paymentMethods, paymentMethod => {
                    items.push({id: paymentMethod.id, name: paymentMethod.name, purchases: paymentMethod.purchases});
                });
                return items;
            }
        },
