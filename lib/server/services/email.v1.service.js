...
module.exports = {
    ...
    events: {
        ...
        'support:created': {
            async handler(ctx) {
                ...
                const supportEmail = _.get(this.broker.metadata, 'support');
                if (!supportEmail) {
                    return;
                }
                ...
                const params = {};
                params.to = {name: i18n.__('support'), address: supportEmail};
                params.subject = i18n.__('support');
                params.html = this.support(name, email, phone, message);
    ...
    methods: {
        ...
        async send(job, done) {
            ...
            const name = _.get(this.broker.metadata, 'name', global.DOMAIN);
            const smtp = _.get(this.broker.metadata, 'smtp');
            const email = _.get(this.broker.metadata, 'email');
