    ...
    events: {
        'order:created': {
            async handler(ctx) {
                ...
                const site = await ctx.call('v1.sites.get', {id: _.toString(order.site), fields: 'user lang'});
                if (!site) {
                    throw new MoleculerServerError('Site not found');
                }
                const user = await ctx.call('v1.users.get', {id: _.toString(site.user), fields: 'name email'});
                if (!user) {
                    throw new MoleculerServerError('User not found');
                }
            ...
            async handler(ctx) {
                ...
                switch (type) {
                    case 'order':
                        ...
                        if (!order) {
                            throw new MoleculerServerError('Order not found', 404);
                        }
                        const site = await ctx.call('v1.sites.get', {id: _.toString(order.site), fields: 'lang'});
                        ...
                        const dateAndTimeFormat = site.lang === 'ru' ? 'D.MM.YYYY' : 'M/D/YYYY';
                        const timeZoneId = site.lang === 'ru' ? 'Europe/Moscow' : 'America/New_York';
                        data.subtitle = `${i18n.__('date')}: ${moment(order.date).tz(timeZoneId).format(dateAndTimeFormat)}`;
                    ...
                    case 'request':
                        ...
                        const message = [];
                        _.each(request.data, item => {
                            message.push(`<strong>${item.label}:</strong> ${item.value}`);
                        });
                        data.subject = ctx.meta.__('new-request');
                        data.message = _.join(message, '<br>');
                        data.image = `${global.DOMAIN}images/email/subscribing.png`;*/


                        const request = await ctx.call('v1.requests.get', {id});
                        if (!request) {
                            throw new MoleculerServerError('Request not found', 404);
                        }
