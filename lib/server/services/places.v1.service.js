        ...
        if (count === 0) {
            ...
            if (city) {
                await this.adapter.model.create({
                    active: true,
                    primary: true,
                    name: `${city.name}, Knickerbocker Ave, 729`,
                    city: city._id,
                    street: 'Knickerbocker Ave',
                    house: '729',
    ...
    actions: {
        ...
        item: {
            ...
            async handler(ctx) {
                ...
                if (!city) {
                    throw new MoleculerServerError('City not found', 404);
                }

                const region = await ctx.call('v1.regions.findOne', {conditions: {_id: city.region}});
                if (!region) {
                    throw new MoleculerServerError('Region not found', 404);
                }

                const country = await ctx.call('v1.countries.findOne', {conditions: {_id: city.country}});
                if (!country) {
                    throw new MoleculerServerError('Country not found', 404);
                }
                ...
                result.street = place.street;
                result.house = place.house;
        ...
        add: {
            ...
            async handler(ctx) {
                ...
                const street = ctx.params.street;
                const house = ctx.params.house;
                ...
                set.pickup = pickup;
                set.inside = inside;
                set.name = `${city.name}, ${street}, ${house}`;
                set.city = cityid;
                set.street = street;
                set.house = house;
