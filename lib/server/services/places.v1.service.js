...
module.exports = {
    ...
    async started() {
        ...
        if (count === 0) {
            ...
            const city = await this.broker.call('v1.cities.findOne', {query: {name: 'New York'}});
            if (city) {
    ...
    actions: {
        ...
        item: {
            ...
            async handler(ctx) {
                ...
                const place = await ctx.call('v1.places.findOne', {query: {_id: id}});
                if (!place) {
                    throw new MoleculerServerError(ctx.meta.__('place-not-found'), 404);
                }

                const city = await ctx.call('v1.cities.findOne', {query: {_id: place.city}});
                if (!city) {
                    throw new MoleculerServerError(ctx.meta.__('city-not-found'), 404);
                }

                const region = await ctx.call('v1.regions.findOne', {query: {_id: city.region}});
                if (!region) {
                    throw new MoleculerServerError(ctx.meta.__('region-not-found'), 404);
                }

                const country = await ctx.call('v1.countries.findOne', {query: {_id: city.country}});
                if (!country) {
        ...
        add: {
            ...
            async handler(ctx) {
                ...
                const city = await ctx.call('v1.cities.findOne', {query: {_id: cityid}});
                if (!city) {
                    throw new MoleculerServerError(ctx.meta.__('city-not-found'), 404);
                }
        ...
        delete: {
            ...
            async handler(ctx) {
                ...
                await ctx.call('v1.places.deleteOne', {query: {_id: id}});
