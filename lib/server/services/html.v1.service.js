    ...
    actions: {
        ...
        index: {
            ...
            async handler(ctx) {
                ...
                const showcase = await ctx.call('v1.showcase.data', {lang});
                ...
                let favicon = _.get(this.broker.metadata, 'favicon');
                if (favicon) {
                    favicon = `/v1/images/${favicon}`;
                } else {
                    favicon = '/favicon.ico';
                }
        ...
        promotionModal: {
            ...
            async handler(ctx) {
                ...
                if (!promotion) {
                    throw new MoleculerServerError('Promotion not found', 404);
                }
                const data = {
                    modalid,
                    name: promotion.name,
        ...
        agreementModal: {
            ...
            async handler(ctx) {
                ...
                if (!agreement) {
                    throw new MoleculerServerError('Agreement not found', 404);
                }
                const data = {
                    modalid,
                    name: agreement.name,
        ...
        profileModal: {
            ...
            async handler(ctx) {
                ...
                if (!customer) {
                    throw new MoleculerServerError('Customer not found', 404);
                }
                const data = {
                    modalid,
        ...
        bonusModal: {
            ...
            async handler(ctx) {
                ...
                const data = {
                    modalid,
                    bonuses: _.get(customer, 'bonuses', 0),
                };
                return {
                    modalid,
        ...
        ordersModal: {
            ...
            async handler(ctx) {
                ...
                const dateAndTimeFormat = ctx.meta.__('date-and-time-format');
                const customerid = ctx.params.customerid;
                ...
                _.each(_orders, order => {
                    ...
                    _.each(order.items, item => {
                        description.push(`${item.name} ${item.quantity} x ${cost(item.amount, lang)} = ${cost(item.quantity * item.amount, lang)}`);
                    });
                    orders.push({
                        id: order.id,
                        date: moment(order.date).format(dateAndTimeFormat),
                        amount: cost(order.amount, lang),
                        paid: order.paid,
        ...
        bonusesModal: {
            ...
            async handler(ctx) {
                ...
                _.each(_bonuses, bonuse => {
                    bonuses.push({
                        id: bonuse.id,
                        date: moment(bonuse.date).format(dateAndTimeFormat),
                        amount: cost(bonuse.amount, lang),
        ...
        balancesModal: {
            ...
            async handler(ctx) {
                ...
                _.each(_balances, balance => {
                    balances.push({
                        id: balance.id,
                        date: moment(balance.date).format(dateAndTimeFormat),
                        amount: cost(balance.amount, lang),
        ...
        wishlistModal: {
            ...
            async handler(ctx) {
                const lang = ctx.meta.getLocale();
                const units = await ctx.call('v1.units.get', {language: lang});
                ...
                if (_.size(_products) === 0) {
                    throw new MoleculerServerError('Products not found', 404);
                }
                const products = [];
        ...
        contactsModal: {
            ...
            async handler(ctx) {
                ...
                if (!place) {
                    throw new MoleculerServerError('Place not found', 404);
                }
        ...
        productModal: {
            ...
            async handler(ctx) {
                ...
                if (!product) {
                    throw new MoleculerServerError('Product not found', 404);
                }
        ...
        modifierModal: {
            ...
            async handler(ctx) {
                ...
                if (!modifier) {
                    throw new MoleculerServerError('Modifier not found', 404);
                }
                const items = [];
        ...
        rechargeModal: {
            ...
            async handler(ctx) {
                ...
                const amounts = await ctx.call('v1.balances.amounts', {lang});
    ...
    methods: {
        async compile(ctx, file, data = {}) {
            ...
            const context = {
                ...
                icon: this.icon,
                cost: (obj) => {
                    let str = cost(_.isArray(obj) ? obj[0].price : obj, lang);
