...
module.exports = {
    ...
    events: {
        ...
        'checklist:deleted': {
            async handler(ctx) {
                ...
                await this.adapter.model.updateMany({'value.checklist': checklist._id}, {'value.active': false});
            }
        },
    },
    async created() {
    ...
    actions: {
        ...
        customer: {
            rest: 'GET /customer/config',
            async handler(ctx) {
                return {
                    lang: global.LANG,
                    name: _.get(this.broker.metadata, 'name'),
                    ip: _.replace(ctx.meta.ip, '::ffff:', ''),
                    colors: _.get(this.broker.metadata, 'colors', colors),
                };
            },
        },
        user: {
        ...
        getBasic: {
            ...
            async handler() {
                const keys = ['name', 'organization', 'authorization', 'initial'];
        ...
        setBasic: {
            ...
            async handler(ctx) {
                ...
                ops.push({
                    updateOne: {
                        filter: {key: 'initial'},
                        update: {value: ctx.params.initial},
                        upsert: true,
                    }
                });
        ...
        getMysteryShopper: {
            role: 'customers:read',
            rest: 'GET /user/config/mystery-shopper',
            async handler(ctx) {
                const mysteryShopper = await this.adapter.model.findOne({key: 'mysteryShopper'});
                ...
                return {
                    mysteryShopper: _.get(mysteryShopper, 'value'),
                    checklists,
                };
            },
        },
        setMysteryShopper: {
            ...
            rest: 'PATCH /user/config/mystery-shopper',
            params: {
                mysteryShopper: 'object',
            },
            async handler(ctx) {
                const value = ctx.params.mysteryShopper;
                value.checklist = mongoose.Types.ObjectId(value.checklist);
                await this.adapter.model.findOneAndUpdate({key: 'mysteryShopper'}, {value}, {upsert: true});
                await this.broker.broadcast('config:changed');
            },
        },
    ...
    methods: {
        async update() {
            ...
            if (!_.get(this.broker.metadata, 'desiredTimes')) {
                _.set(this.broker.metadata, 'desiredTimes', [5, 10, 15, 20, 25, 30]);
            }
            if (!_.get(this.broker.metadata, 'amounts')) {
                let amounts;
                if (global.LANG === 'ru') {
                ...
                _.set(this.broker.metadata, 'amounts', amounts);
            }
            if (!_.get(this.broker.metadata, 'colors')) {
                _.set(this.broker.metadata, 'colors', colors);
            }
