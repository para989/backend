    ...
    actions: {
        ...
        image: {
            async handler(ctx) {
                ...
                const imageid = _.get(ctx.meta.headers, 'type',  new mongoose.Types.ObjectId());
                const ext = fileExtension(ctx.meta.filename);
                const regularName = `${imageid}.${ext}`;
                ...
                switch (imageid) {
                    case 'logo':
                        await ctx.call('v1.config.updateOne', {
                            filter: {key: 'logo'},
                            doc: {value: regularName},
                            options: {upsert: true},
                        });
                        break;
                    case 'banner':
                ...
                return {file: regularName, picture: regularName, width, height};
        ...
        svg: {
            async handler(ctx) {
                ...
                await ctx.call('storage.put', {key: `images/${imageid}.svg`, body: svg});

                return {file: `${imageid}.svg`};
        ...
        banners: {
            ...
            async handler(ctx) {
                ...
                const filePath = `${os.tmpdir()}/content/banners/${file}`;
                fs.ensureDirSync(path.dirname(filePath));
                const obj = await ctx.call('storage.get', {key, filePath}).catch(() => {
                    throw new MoleculerServerError(ctx.meta.__('file-not-found'), 404);
                });
        ...
        code: {
            ...
            async handler(ctx) {
                ...
                return qrImage.imageSync(`STARTQR${code}ENDQR`, {type: 'svg'});
            }
        },
        // v1.images.maps
        maps: {
