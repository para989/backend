...
const use = [
    async function (req, res, next) {
        ...
        if (_.isBuffer(req.body)) {
            req.$ctx.meta.text = req.body.toString();
        }
        next();
    }
];
...
module.exports = {
    ...
    methods: {
        async authenticate(ctx, route, req/*, res*/) {
            ...
            const pass = _.get(credentials, 'pass');
            if (name === 'customer') {
                const arr = _.split(pass, '-');
                if (_.size(arr) === 2) {
                    const id = arr[0];
                    const token = arr[1];
            ...
            } else if (mongoose.Types.ObjectId.isValid(name) && pass) {
                const user = await ctx.call('v1.users.check', {id: name, token: pass});
                if (_.size(user)) {
