    ...
    actions: {
        ...
        selector: {
            ...
            async handler(ctx) {
                ...
                const products = await this.adapter.model.find({active: true}, 'id name gallery groups').sort('order').exec();
                ...
                _.each(products, product => {
                    ...
                    _.each(product.groups, groupid => {
                        if (_.isUndefined(map[groupid])) {
                            map[groupid] = [];
                        }
                        delete product.groups;
                        map[groupid].push(product);
                    });
                });

                const result = [];

                const groups = await ctx.call('v1.groups.find', {query: {active: true}, fields: 'id name', sort: 'order'});
                _.each(groups, group => {
                    // group = group.toJSON();
                    if (_.size(map[group.id])) {
                        group.products = map[group.id];
                        result.push(group);
                    }
                });
        ...
        popular: {
            role: 'reports:read',
            rest: 'GET /user/popular/products',
            cache: true,
            async handler(ctx) {
