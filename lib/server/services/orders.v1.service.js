    ...
    actions: {
        ...
        createReport: {
            ...
            async handler(ctx) {
                ...
                const addresses = {};
                _.each(places, place => {
                    addresses[place.id] = place.address;
                });
                ...
                if (_.size(orders) || _.size(payments)) {
                    ...
                    const _paymentMethods = await ctx.call('v1.payment-methods.find', {fields: 'id name type'});
                    const paymentMethods = {};
                    _.each(_paymentMethods, paymentMethod => {
                        paymentMethods[paymentMethod.id.toString()] = paymentMethod;
                    });
                    ...
                    _.each(orders, order => {
                        if (!order.test) {
                            _.each(order.items, item => {
                                ...
                                let discount = _.get(item, 'discount', 0);

                                const paymentMethod = paymentMethods[order.paymentMethod.toString()];
                                if (paymentMethod.type === 'balance' || paymentMethod.type === 'bonuses') {
                                    discount = amount;
                                    amount = 0;
                                }
                                ...
                                result.items.push({
                                    ...
                                    date: save ? format(order.created, dateFormat) : order.created,
                                    name: item.name,
                                    place: addresses[order.place.toString()],
                                    quantity: item.quantity,
                                    amount,
                                    discount: discount > 0 ? discount : 0,
                                    paymentMethod: item.giftid ? ctx.meta.__('gift') : paymentMethod.name,
                                });

                                _.each(item.modifiers, modifier => {

                                    let modifierAmount = modifier.amount;
                                    let modifierDiscount = 0
                                    if (paymentMethod.type === 'balance' || paymentMethod.type === 'bonuses') {
                                        modifierDiscount = modifierAmount;
                                        modifierAmount = 0;
                                    }
                                    result.amount += modifierAmount;
                    ...
                    _.each(payments, payment => {
                        ...
                        result.items.push({
                            ...
                            date: save ? format(payment.created, dateFormat) : payment.created,
                            name: ctx.meta.__(payment.cause),
                            place: addresses[payment.place.toString()],
                            quantity: 1,
                            amount: payment.amount,
                            discount: 0,
                            paymentMethod: _.get(paymentMethods, [_.toString(payment.paymentMethod), 'name'], ''),
                        });
                    });
