    ...
    actions: {
        ...
        add: {
            async handler(ctx) {
                ...
                const place = await ctx.call('v1.places.get', {id: placeid, fields: 'test'});
                ...
                const test = _.get(ctx.params, 'test') || _.get(place, 'test') || _.get(customer, 'test');
                ...
                if (_.size(orders) || _.size(payments)) {
                    ...
                    _.each(payments, payment => {
                        ...
                        result.items.push({
                            ...
                            place: places[payment.place.toString()],
                            quantity: 1,
        ...
        test: {
            async handler(ctx) {
                ...
                const customer = await ctx.call('v1.customers.findOne', {conditions: {test: true}});
                if (!customer) {
